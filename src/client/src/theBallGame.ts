...
export default class TheBallGame {
  ...
  constructor(
      ...
      (object: THREE.Group) => {
        object.traverse(function (child) {
          if ((child as THREE.Mesh).isMesh) {
            (child as THREE.Mesh).material = new THREE.MeshMatcapMaterial({
    ...
    socket.on('joined', (id: string, screenName: string, recentWinners: []) => {
      this.myId = id;
      (document.getElementById('screenNameInput') as HTMLInputElement).value =
        screenName;
    ...
    socket.on('gameData', (gameData: any) => {
      if (gameData.gameClock >= 0) {
        ...
        (document.getElementById('gameClock') as HTMLDivElement).innerText =
          Math.floor(gameData.gameClock).toString();
      } else {
        if (this.jewel) {
          this.jewel.visible = false;
        }
        (document.getElementById('gameClock') as HTMLDivElement).style.display =
          'none';
        if (
          !this.ui.menuActive &&
                ...
                .onUpdate(() => {
                  this.camera.position.set(
                    this.players[p].position.x +
                      this.radius * Math.cos(this.cameraRotationXZOffset),
                    this.players[p].position.y +
                      this.radius * Math.atan(this.cameraRotationYOffset),
                    this.players[p].position.z +
                      this.radius * Math.sin(this.cameraRotationXZOffset)
                  );
                  this.camera.lookAt(
